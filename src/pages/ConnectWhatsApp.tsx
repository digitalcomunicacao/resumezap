...
const ConnectWhatsApp = () => {
  ...
  // controla se o QR já foi gerado
  ...
  // ============================================================
  // 🔵 GERAR QR CODE — Apenas quando usuário clicar no botão
  // ============================================================
  ...
  const generateQrCode = async () => {
    ...
    try {
      ...
      // Caso usuário já tenha WhatsApp conectado no passado
      if (data?.connected) {
      ...
      const formattedQr = data.qrCode.startsWith("data:") ? data.qrCode : `data:image/png;base64,${data.qrCode}`;

      setQrCode(formattedQr);
      setInstanceId(data.instanceId);
      setQrGenerated(true);

      toast.message("QR Code gerado! Escaneie com seu WhatsApp.");

      // 🔥 VERIFICA APENAS UMA VEZ APÓS 30 SEGUNDOS
      setTimeout(() => {
        checkStatus();
      }, 30000);
    } catch (err: any) {
  ...
  // ============================================================
  // 🟣 CHECAR STATUS — Apenas quando chamado (sem loops)
  // ============================================================
  ...
  const checkStatus = async () => {
    ...
    try {
      ...
      if (data.status === "connected") {
        ...
        toast.error("Ainda não conectado. Escaneie novamente ou tente outra vez.");
      }
    } catch (err: any) {
              ...
              <Card className="shadow-soft">
                ...
                <CardContent className="space-y-4">
                  ...
                  <Step
                    ...
                    desc="Toque em Mais opções (⋮) → Aparelhos conectados"
                  />
                ...
                <CardContent className="flex flex-col items-center gap-6 pb-8">
                  ...
                  {qrCode && qrGenerated && !error && (
                    <>
                      ...
                      {/* Botão manual para verificar novamente */}
                      {!checking && (
